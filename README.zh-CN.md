# Jinping (Cimy) Wang, Ph.D.

[English](./README.md) | [中文](./README.zh-CN.md)

**讲师 | 多模态数据融合、计算机视觉、** | 广东技术师范大学人工智能学院


[![Google Scholar](https://img.shields.io/badge/Google_Scholar-Profile-blue?style=flat&logo=google-scholar)](https://scholar.google.com/citations?hl=zh-CN&user=sYT7hPwAAAAJ&view_op=list_works&sortby=pubdate)
[![Homepage](https://img.shields.io/badge/GPNU-Profile-green?style=flat&logo=google-scholar)](https://jkxy.gpnu.edu.cn/info/1248/6759.htm)
[![Institutional Email](https://img.shields.io/badge/Institutional_Email-wangjp@gpnu.edu.cn-red?style=flat&logo=minutemailer)](mailto:wangjp@gpnu.edu.cn)
[![Personal Email](https://img.shields.io/badge/Personal_Email-jinping_wang@foxmail.com-yellow?style=flat&logo=gmail)](mailto:jinping_wang@foxmail.com)
[![Citations](https://img.shields.io/endpoint?logo=Google%20Scholar&url=https%3A%2F%2Fgoogle-scholar-badge.vercel.app%2Fcitations%3Fuser%3DsYT7hPwAAAAJ&color=blue)](https://scholar.google.com/citations?user=sYT7hPwAAAAJ)
![Visitors](http://views.whatilearened.today/views/github/Cimy-wang/views.svg)

<div align="center">
  <img alt="GIF"
       src="https://raw.githubusercontent.com/JoeyBling/JoeyBling/master/pic/pusheencode.gif" />
</div>

<!-- PROFILE-STATS:START -->
## 📊 学术成果概览

<p align="center">
  <img alt="论文: 16" src="https://img.shields.io/badge/%E8%AE%BA%E6%96%87-16-2F80ED?style=flat-square" />
  <img alt="主持项目: 6" src="https://img.shields.io/badge/%E4%B8%BB%E6%8C%81%E9%A1%B9%E7%9B%AE-6-059669?style=flat-square" />
  <img alt="发明专利: 4" src="https://img.shields.io/badge/%E5%8F%91%E6%98%8E%E4%B8%93%E5%88%A9-4-EA580C?style=flat-square" />
  <img alt="软件著作权: 2" src="https://img.shields.io/badge/%E8%BD%AF%E4%BB%B6%E8%91%97%E4%BD%9C%E6%9D%83-2-7C3AED?style=flat-square" />
  <img alt="学生获奖: 11" src="https://img.shields.io/badge/%E5%AD%A6%E7%94%9F%E8%8E%B7%E5%A5%96-11-CA8A04?style=flat-square" />
  <img alt="主讲课程: 3" src="https://img.shields.io/badge/%E4%B8%BB%E8%AE%B2%E8%AF%BE%E7%A8%8B-3-0F766E?style=flat-square" />
  <img alt="开源工具: 1" src="https://img.shields.io/badge/%E5%BC%80%E6%BA%90%E5%B7%A5%E5%85%B7-1-475467?style=flat-square" />
</p>

<p align="center"><em>由 GitHub Actions 根据本页内容自动更新。</em></p>
<!-- PROFILE-STATS:END -->

---

## 🏛️ 个人简介

现任广东技术师范大学人工智能学院讲师，获中山大学电子信息博士学位。

主要研究**计算机视觉**与**多模态数据融合**的交叉领域在**具身智能（EAI）\车路协同（V2X）\遥感（RS）**方面的应用。致力于面向复杂真实开放场景，构建鲁棒的感知系统。

我与来自*中山大学*、*剑桥大学*、*鹏城实验室*的研究人员保持积极、长期的科研合作。

---

## 📌 主持项目

- 环境噪声与通信受限共扰下的路侧多视角三维协同感知方法研究，国家自然科学基金青年科学基金项目（2027.1–2029.12）
- 多智能体协同可信自主感知算法研究，专项科研项目（2027.1–2027.12）
- 基于 VAE+Transformer 结构的用电检查健康诊断模型研究，企业委托（2026.7–2026.12）
- 工业动态视觉追踪算法软件，企业委托（2025.6–2025.10）
- 多模态异构特征融合及关键技术研究，专项科研项目（2024.10-2025.10）
- 高光谱遥感图像深层空谱特征提取方法及洞庭湖水域动态监测研究，湖南省研究生重点项目（2018.1-2018.12）

---

## 🔬 代表性论文与学术成果

*本人秉持开放科学的研究理念，在数据合理条件下，会公开发表论文相关的数据集和代码库。如需获取完整且最新的论文列表，请访问我的 **[Google Scholar Profile](https://scholar.google.com/citations?hl=zh-CN&user=sYT7hPwAAAAJ&view_op=list_works&sortby=pubdate)**。*

### 2026
- [**CVPR '26** <span style="color:red;font-weight:bold">🏆 Highlight</span>] Seeing the Scene Matters: Revealing Forgetting in Video Understanding Models with a Scene-Aware Long-Video Benchmark. Seng Nam Chen, Hao Chen, Chenglam Ho, Xinyu Mao, **Jinping Wang,** Yu Zhang, Chao Li.
 
  *提出 SceneBench，用于评估 VLM 的场景级长视频理解能力；并提出 Scene-RAG，通过动态场景记忆有效缓解长上下文遗忘。* [🔗 Paper](https://arxiv.org/abs/2603.27259) [🔗 Dataset & Code](https://huggingface.co/datasets/SinerChen/SceneBench)

- **[IEEE TMM '26]** Cognidrive: Cognitive Autonomous Driving Understanding with Multistep Multimodal Chain-of-Thought Reasoning. Xiangyi Qin, Xiaofei Zhang, Shuai Wang, Yuzhen Wei, **Jinping Wang\***, Xiaojun Tan.

  *该论文通过多步多模态思维链推理，将复杂驾驶场景的环境感知、语义理解与决策推理有机结合，提升自动驾驶系统的认知能力与可解释性。* 📖 [🔗 Paper](https://ieeexplore.ieee.org/abstract/document/11548610/) 

- **[Information Fusion '26]** Inscope: A new real-world 3d infrastructure-side collaborative perception dataset for open traffic scenarios, Xiaofei Zhang#, 
Yining Li#, **Jinping Wang#**, Xiangyi Qin, Ying Shen, Zhengping Fan, Xiaojun Tan

  *构建大规模真实场景数据集，涵盖多位置 LiDAR，旨在解决 I2I 感知系统中的遮挡挑战。*  [🔗 Paper](https://www.sciencedirect.com/science/article/pii/S1566253525010139) [🔗 Dataset & Code](https://github.com/xf-zh/InScope)

- **[ADVEI '26]** Confidence-V2X: Confidence-driven sparse communication for efficient V2X cooperative perception. Xiaojun Tan, Rui Wang, **Jinping Wang\***, Shuai Wang, Xu Wang, Dongsheng Wu.

  *提出置信度感知的协同感知框架，在 V2X 系统中联合优化目标检测性能与通信效率。* [🔗 Paper](https://www.sciencedirect.com/science/article/pii/S1474034625008079) [🔗 Code](https://github.com/Rwang0208/Confidence-V2X)

### 2024 - 2025

- **[IEEE TGRS '25]** FusDreamer: Label-efficient remote sensing world model for multimodal data classification. **Jinping Wang**, Weiwei Song, Hao Chen, Jinchang Ren, Huimin Zhao.

  *面向多模态数据融合的标签高效遥感世界模型，探索世界模型在遥感领域的应用潜力。* [🔗 Paper](https://ieeexplore.ieee.org/document/10942442/) [🔗 Code](https://github.com/Cimy-wang/FusDreamer)

- **[IEEE GRSL '25]** CaPaT: Cross-Aware Paired-Affine Transformation for Multimodal Data Fusion Network. **Jinping Wang**, Hao Chen, Xiaofei Zhang, Weiwei Song. 

  *提出直接特征交互范式，在显著减少模型参数的同时提升特征融合的迁移效率。* [🔗 Paper](https://ieeexplore.ieee.org/document/10973085/) 

- **[ICASSP '24]** BEVLOC: End-to-end 6-dof localization via cross-modality correlation under bird’s eye view. Nanjie Chen, **Jinping Wang**, Hao Chen, Ying Shen, Shuai Wang, Xiaojun Tan

  *一种端到端车辆定位方法：通过基于光流的跨模态相关性，在 BEV 空间融合单目图像与 LiDAR 地图特征。*  [🔗 Paper](https://ieeexplore.ieee.org/document/10445984/)

### 2022 - 2023
- **[IEEE TCSVT '23]** Mutually beneficial transformer for multimodal data fusion. **Jinping Wang**, Xiaojun Tan.

  *引入动态区域感知卷积，用于空间引导掩码生成与高程显著性智能体引导。*  [🔗 Paper](https://ieeexplore.ieee.org/document/10122197/)

- **[IEEE TCSVT '22]** AM³Net: Adaptive mutual-learning-based multimodal data fusion network. **Jinping Wang**, Jun Li, Yanli Shi, Jianhuang Lai, Xiaojun Tan.

  *通过协同特征传递，关注 HSI 光谱通道的特异性以及 HSI 与 LiDAR 空间信息的互补性。*  [🔗 Paper](https://ieeexplore.ieee.org/document/9698196/) [🔗 Code](https://github.com/Cimy-wang/AM3Net)
  
- **[IEEE ICASSP '22]** Spectral-spatial symmetrical aggregation cross-linking multi-modal data fusion network. **Jinping Wang**, Jun Li, Xiaojun Tan.

  *开发采用 involution 运算和金字塔特征融合的 SACLNet，用于鲁棒的多模态数据分类。*  [🔗 Paper](https://ieeexplore.ieee.org/document/9747570/)

- **[Neurocomputing '22]** ASPCNet: Deep adaptive spatial pattern capsule network for hyperspectral image classification. **Jinping Wang**, Xiaojun Tan, Jianhuang Lai, Jun Li.

  *一种基于扩展的语义自适应感受野的自适应空间模式胶囊网络架构。*   [🔗 Paper](https://www.sciencedirect.com/science/article/pii/S092523122200220X)  [🔗 Code](https://github.com/Cimy-wang/ASPCNet)


### 2020 - 2021
- **[Electronics '21]** A simulated annealing algorithm and grid map-based UAV coverage path planning method for 3D reconstruction. Sichen Xiao, Xiaojun Tan, **Jinping Wang**.

  *提出兼顾图像重叠率与能耗效率的 UAV CPP 框架，并通过现场实验进行验证。*  [🔗 Paper](https://www.mdpi.com/2079-9292/10/7/853)

### 2018 - 2019
- **[IEEE TGRS '19]** Spatial density peak clustering for hyperspectral image classification with noisy labels. Bing Tu, Xiaofei Zhang, Xudong Kang, **Jinping Wang**, Jón Atli Benediktsson.

  *提出基于空间密度峰值（SDP）聚类的方法，用于检测和处理 HSI 训练集中的错误标注样本。*   [🔗 Paper](https://ieeexplore.ieee.org/document/8732685/)  [🔗 Code](https://github.com/xf-zh/Spatial-Density-Peak-Clustering-for-Hyperspectral-Image-Classification-with-Noisy-Labels)

- **[IEEE JSTARS '19]** Texture pattern separation for hyperspectral image classification. Bing Tu#, **Jinping Wang#**, Guoyun Zhang, Xiaofei Zhang, Wei He.

  *通过一种新的 TPS 特征提取方法，解决 HSI 中的层分离问题。*  [🔗 Paper](https://ieeexplore.ieee.org/document/8846581/)  [🔗 Code](https://github.com/Cimy-wang/TPS_HSIC)

- **[IEEE JSTARS '18]** KNN-based representation of superpixels for hyperspectral image classification. Bing Tu#, **Jinping Wang#**, Xudong Kang, Guoyun Zhang, Xianfeng Ou, Longyuan Guo.

  *探索两种 k 值选择规则，以确定超像素最具代表性的样本表征。* [🔗 Paper](https://ieeexplore.ieee.org/document/8515116/)  [🔗 Code](https://github.com/Cimy-wang/KNN-based-Representation-of-Superpixels-for-hyperspectral-image-classification)

- **[IEEE GRSL '18]** Hyperspectral image classification via fusing correlation coefficient and joint sparse representation. Bing Tu, Xiaofei Zhang, Xudong Kang, Guoyun Zhang, **Jinping Wang**, Jianhui Wu.

  *一种融合相关系数与联合稀疏表示的高光谱图像分类方法。* [🔗 Paper](https://ieeexplore.ieee.org/document/8268655/)  [🔗 Code](https://github.com/xf-zh/Hyperspectral-Image-Classification-via-Fusing-Correlation-Coefficient-and-Joint-Sparse-Representatio)

---

## 💡 知识产权（第一发明人）

### 发明专利

1. 基于自适应空间模式的深度胶囊网络图像分类方法及系统，专利号：CN112766340B，2024.06.04。（已授权）
2. 一种面向多模态数据融合的小样本遥感世界模型，专利号：202510038010.2，2025.01.10。（已受理）
3. 一种多模态数据融合的跨域掩码自编码器方法及系统，专利号：202510960900.9，2025.07.12。（已受理）
4. 一种特征自适应互引导的多源信息融合分类方法及系统，专利号：CN114187526B，2025.04.29。（已授权）

### 软件著作权

1. 多模态遥感图像处理系统，登记号：2025R11L0356652，2025.03.10。
2. 基于深度学习的垃圾分类识别系统，登记号：2025SR1736527，2025.09.09。

---

## 🔎 学术服务

- 担任 `IEEE TMM`、`IEEE TCSVT`、`IEEE TGRS`、`IEEE JSTARS`、`Knowledge-Based Systems (KBS)`、`CVPR` 和 `IEEE TIM` 审稿人。

---

## 🏫 主讲课程


- 人工智能导论
- 计算机网络
- 人工智能与信息社会

## 🏆 指导学生获奖

| 年份 | 竞赛 | 获奖等级 |
| --- | --- | --- |
| 2025 | 蓝桥杯（Python B 组） | 省级 / 二等奖 |
| 2025 | 未来杯大数据挑战赛 | 国家级 / 三等奖 |
| 2025 | MathorCup 高校数学建模挑战赛（货运量预测） | 国家级 / 成功参赛奖 |
| 2025 | 中国电机工程学会杯 | 国家级 / 成功参赛奖 |
| 2025 | 数维杯数学建模竞赛（春季赛） | 国家级 / 二等奖 |
| 2025 | APMCM 亚太地区大学生数学建模竞赛（中文赛） | 国家级 / 三等奖 |
| 2025 | 全国大学生仿真建模大赛 | 国家级 / 二等奖 |
| 2025 | 数维杯数学建模竞赛（秋季赛） | 国家级 / 一等奖 |
| 2024 | 美国大学生数学建模竞赛（MCM） | 国际级 / 成功参赛奖 |
| 2024 | MathorCup 高校数学建模大数据挑战赛 | 国家级 / 成功参赛奖 |
| 2024 | APMCM 亚太地区大学生数学建模竞赛（宠物产业） | 国家级 / 三等奖 |

---

## 🧰 开源工具

- **[Cimy_PPtools](https://github.com/Cimy-wang/Cimy_PPtools)**：面向高光谱分类与融合任务的综合 Python 工具箱，支持数据预处理、模型序列化和结果可视化。

**核心技术栈：**
`Python` | `MATLAB` | `Shell` | `LaTeX`

---
*✉️ 欢迎开展科研合作与学术交流，敬请联系。*
